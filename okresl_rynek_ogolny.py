...
Dodatkowo buduje raport stanu rynku i poszczególnych aktywów
w osobnym pliku JSON, np. market_state_report.json.

Użycie:
python okresl_rynek.py [--data-dir KATALOG] [--output PLIK] [--state-output PLIK] [--verbose]
"""
...
def score_all(data: dict):
    ...
    scores["crypto_trend_score"] = (
        float(sum(trend_values) / len(trend_values)) if trend_values else None
    )
    ...
    # --- Derywatywy ---
    # Uwaga: rola taker ratio jest zapisana jako sol_taker_1h,
    # zgodnie z ROLE_PATTERNS.
    derivatives_pairs = []
    ...
    sol_taker = get_series(data, "sol_taker_1h")
    ...
    # --- Tagi ---
    if (
        scores.get("sol_relative_strength_score") is not None
        and scores["sol_relative_strength_score"] < -0.20
    ):
    ...
    if (
        scores.get("eth_relative_strength_score") is not None
        and scores["eth_relative_strength_score"] > 0.20
    ):
    ...
    if (
        scores.get("volatility_stress_score") is not None
        and scores["volatility_stress_score"] > 0.60
    ):
        tags.append("volatility_high")

    if (
        scores.get("volatility_stress_score") is not None
        and scores["volatility_stress_score"] < 0.20
    ):
...
def build_output(
    ...
    missing_datasets = [
        q["role"] for q in quality if q["status"] in ("MISSING_FILE", "MISSING")
    ]
...
# ============================================================
# 9. RAPORT STANU RYNKU I POSZCZEGÓLNYCH AKTYWÓW
# ============================================================

def round_or_none(value, digits=6):
    """Bezpieczne zaokrąglanie do JSON-a."""
    if value is None:
        return None
    try:
        if pd.isna(value):
...
def interpret_trend_state(score):
    """Interpretacja trendu na podstawie score'a."""
    if score is None:
        return "unknown"
    if score >= 0.30:
...
def interpret_short_return(ret):
    """Interpretacja krótkoterminowego ruchu, głównie dla danych godzinowych."""
    if ret is None:
        return "unknown"
    if ret >= 0.05:
        return "strong_up"
...
def interpret_macro_state(score):
    """Interpretacja makro: risk-on / risk-off."""
    if score is None:
        return "unknown"
    if score >= 0.30:
        return "risk_on"
...
def interpret_liquidity_state(score):
    ...
    if score >= 0.20:
        return "expanding"
    if score >= 0.05:
        return "mildly_expanding"
    if score <= -0.20:
        return "contracting"
    if score <= -0.05:
        return "mildly_contracting"
    return "stable"


def interpret_volatility_state(score):
...
def interpret_funding_z(z):
    """Interpretacja z-score funding rate."""
    if z is None:
        return "unknown"
    if z >= 2.0:
        return "very_overheated_longs"
    if z >= 1.0:
        return "overheated_longs"
    if z >= 0.40:
        return "positive_funding"
    if z <= -2.0:
        return "extreme_short_pressure"
    if z <= -1.0:
...
def interpret_oi_z(z):
    """Interpretacja z-score zmian open interest."""
    if z is None:
        return "unknown"
    if z >= 1.0:
...
def interpret_taker_ratio(ratio):
    """Interpretacja taker ratio."""
    if ratio is None:
        return "unknown"
    if ratio >= 1.08:
...
def interpret_taker_z(z):
    """Interpretacja taker ratio przez z-score, jeśli brak ostatniej wartości."""
    if z is None:
        return "unknown"
    if z >= 2.0:
        return "aggressive_buy"
...
ASSET_DISPLAY_NAMES = {
    "btc_usd_1h": "BTC/USD 1h",
    "btc_usd_1d": "BTC/USD 1d",
    "eth_usd_1h": "ETH/USD 1h",
    "eth_usd_1d": "ETH/USD 1d",
    "sol_usd_1h": "SOL/USD 1h",
    "sol_usd_1d": "SOL/USD 1d",
    "sol_btc_1h": "SOL/BTC 1h",
    "sol_btc_1d": "SOL/BTC 1d",
    "eth_btc_1h": "ETH/BTC 1h",
    "eth_btc_1d": "ETH/BTC 1d",
    "btc_perp_1h": "BTC PERP 1h",
    "eth_perp_1h": "ETH PERP 1h",
    "sol_perp_1h": "SOL PERP 1h",
    "btc_funding_1h": "BTC Funding 1h",
    "eth_funding_1h": "ETH Funding 1h",
    "sol_funding_1h": "SOL Funding 1h",
    "btc_oi_1h": "BTC Open Interest 1h",
    "eth_oi_1h": "ETH Open Interest 1h",
    "sol_oi_1h": "SOL Open Interest 1h",
    "btc_taker_1h": "BTC Taker Ratio 1h",
    "eth_taker_1h": "ETH Taker Ratio 1h",
    "sol_taker_1h": "SOL Taker Ratio 1h",
    "btc_dominance_1d": "BTC Dominance 1d",
    "eth_dominance_1d": "ETH Dominance 1d",
    "sol_dominance_1d": "SOL Dominance 1d",
    "total_mcap_1d": "Total Market Cap 1d",
    "total_ex_btc_1d": "Total Market Cap ex-BTC 1d",
    "usdt_supply_1d": "USDT Supply 1d",
    "usdc_supply_1d": "USDC Supply 1d",
    "dxy_1d": "DXY 1d",
    "gold_1d": "Gold 1d",
    "nasdaq100_1d": "Nasdaq 100 1d",
    "sp500_1d": "S&P 500 1d",
    "usdjpy_1d": "USD/JPY 1d",
    "vix_1d": "VIX 1d",
    "us10y_1d": "US 10Y 1d",
}


def asset_display_name(role):
    """Zwraca czytelną nazwę aktywa."""
    return ASSET_DISPLAY_NAMES.get(role, role.replace("_", " ").upper())


def asset_group_from_role(role):
    """Przypisuje rolę do grupy tematycznej."""
    if role.endswith("_funding_1h"):
        return "derivatives_funding"

    if role.endswith("_oi_1h"):
        return "derivatives_open_interest"

    if role.endswith("_taker_1h"):
        return "derivatives_taker"

    if role.endswith("_perp_1h"):
        return "derivatives_perp"

    if role.endswith("_dominance_1d"):
        return "dominance"

    if role in {"usdt_supply_1d", "usdc_supply_1d"}:
        return "stablecoins_liquidity"

    if role in {"total_mcap_1d", "total_ex_btc_1d"}:
        return "crypto_market_breadth"
    ...
    if role in {"eth_btc_1h", "eth_btc_1d", "sol_btc_1h", "sol_btc_1d"}:
        return "crypto_relative_strength"

    if role.startswith(("btc_usd", "eth_usd", "sol_usd")):
        return "crypto_spot"

    return "other"


def summarize_asset(data, scores, role):
    """
    Buduje podsumowanie pojedynczego aktywa:
    ostatnia wartość, zmiany, SMA, trend/stan, grupa.
    """
    group = asset_group_from_role(role)
    name = asset_display_name(role)

    series = get_series(data, role)
    df = get_ohlcv(data, role)
    ...
    interval = "1h" if role.endswith("_1h") else "1d"

    last = safe_last(series)

    change_24h = safe_return(series, 24) if interval == "1h" else None
    change_1d = safe_return(series, 1) if interval == "1d" else None

    if interval == "1h":
        change_7d = safe_return(series, 168)
        change_30d = safe_return(series, 720)
    else:
        change_7d = safe_return(series, 7)
        change_30d = safe_return(series, 30)

    sma20 = safe_sma(series, 20)
    sma50 = safe_sma(series, 50)

    trend_score = None
    state = "unknown"
    extra = {}

    # Pochodne mają własne stany
    if group == "derivatives_funding":
        z = compute_zscore(series)
        extra["zscore"] = round_or_none(z, 4)
        state = interpret_funding_z(z)

    elif group == "derivatives_open_interest":
        oi_change = series.diff().dropna()
        z = compute_zscore(oi_change) if len(oi_change) > 10 else None
        extra["change_zscore"] = round_or_none(z, 4)
        state = interpret_oi_z(z)

    elif group == "derivatives_taker":
        z = compute_zscore(series)
        extra["zscore"] = round_or_none(z, 4)

        if last is not None:
            state = interpret_taker_ratio(last)
        else:
            state = interpret_taker_z(z)

    elif group == "derivatives_perp":
        atr, atr_pct = compute_atr(df) if df is not None else (None, None)
        extra["atr"] = round_or_none(atr, 8)
        extra["atr_percentile"] = round_or_none(atr_pct, 2)

        state = interpret_volatility_state(
            atr_pct / 100.0 if atr_pct is not None else None
        )

    else:
        # Dla głównych dziennych serii użyj gotowych score'ów, jeśli istnieją
        if role == "btc_usd_1d":
            trend_score = scores.get("btc_trend_score")
        elif role == "eth_usd_1d":
            trend_score = scores.get("eth_trend_score")
        elif role == "sol_usd_1d":
            trend_score = scores.get("sol_trend_score")
        elif role == "eth_btc_1d":
            trend_score = scores.get("eth_relative_strength_score")
        elif role == "sol_btc_1d":
            trend_score = scores.get("sol_relative_strength_score")
        elif interval == "1d":
            trend_score = compute_simple_trend(series)

        # Dla danych godzinowych użyj prostego stanu z 24h
        if interval == "1h":
            state = interpret_short_return(change_24h)
        else:
            state = interpret_trend_state(trend_score)
    ...
    out = {
        ...
        "interval": interval,
        "available": True,
        "last": round_or_none(last, 8),
        "change_24h": round_or_none(change_24h, 6),
        "change_1d": round_or_none(change_1d, 6),
        "change_7d": round_or_none(change_7d, 6),
        "change_30d": round_or_none(change_30d, 6),
        "sma20": round_or_none(sma20, 8),
        "sma50": round_or_none(sma50, 8),
        "trend_score": round_or_none(trend_score, 4),
        "state": state,
    }
...
def collect_assets_by_group(data, scores):
    """Zbiera wszystkie dostępne aktywa i grupuje je."""
    groups = {}

    for role in sorted(data.keys()):
        summary = summarize_asset(data, scores, role)
        groups.setdefault(summary["group"], []).append(summary)

    return groups


def collect_derivatives_detail(data, scores):
    """
    Szczegółowy raport o pochodnych dla BTC/ETH/SOL:
    funding, open interest, taker ratio.
    """
    symbols = ["btc", "eth", "sol"]
    per_symbol = {}
    ...
    for sym in symbols:
        funding = get_series(data, f"{sym}_funding_1h")
        oi = get_series(data, f"{sym}_oi_1h")
        taker = get_series(data, f"{sym}_taker_1h")

        funding_last = safe_last(funding)
        funding_z = compute_zscore(funding)
        if funding_z is not None:
            funding_zs.append(funding_z)

        oi_last = safe_last(oi)
        oi_z = None
        if oi is not None and len(oi) > 10:
            oi_z = compute_zscore(oi.diff().dropna())
        if oi_z is not None:
            oi_zs.append(oi_z)

        taker_last = safe_last(taker)
        taker_z = compute_zscore(taker)

        if taker_last is not None:
            taker_ratios.append(taker_last)

        if taker_z is not None:
            taker_zs.append(taker_z)

        if taker_last is not None:
            taker_state = interpret_taker_ratio(taker_last)
        else:
            taker_state = interpret_taker_z(taker_z)

        per_symbol[sym.upper()] = {
            "funding_rate_last": round_or_none(funding_last, 8),
            "funding_zscore": round_or_none(funding_z, 4),
            "funding_state": interpret_funding_z(funding_z),
            "open_interest_last": round_or_none(oi_last, 8),
            "open_interest_change_zscore": round_or_none(oi_z, 4),
            "open_interest_state": interpret_oi_z(oi_z),
            "taker_ratio_last": round_or_none(taker_last, 6),
            "taker_zscore": round_or_none(taker_z, 4),
            "taker_state": taker_state,
        }

    avg_funding_z = float(np.mean(funding_zs)) if funding_zs else None
    avg_oi_z = float(np.mean(oi_zs)) if oi_zs else None
    avg_taker_ratio = float(np.mean(taker_ratios)) if taker_ratios else None
    avg_taker_z = float(np.mean(taker_zs)) if taker_zs else None

    funding_state = interpret_funding_z(avg_funding_z)
    oi_state = interpret_oi_z(avg_oi_z)

    if avg_taker_ratio is not None:
        taker_state = interpret_taker_ratio(avg_taker_ratio)
    else:
        taker_state = interpret_taker_z(avg_taker_z)

    # Prosty bias pochodnych
    bias = "neutral"

    if (
        funding_state in ("overheated_longs", "very_overheated_longs")
        and taker_state in ("buy_pressure", "aggressive_buy")
    ):
        bias = "long_overheated"

    elif (
        funding_state in ("negative_funding", "short_pressure", "extreme_short_pressure")
        and taker_state in ("sell_pressure", "aggressive_sell")
    ):
        bias = "short_pressure"

    elif taker_state in ("buy_pressure", "aggressive_buy"):
        bias = "buy_pressure"

    elif taker_state in ("sell_pressure", "aggressive_sell"):
        bias = "sell_pressure"

    elif funding_state in ("overheated_longs", "very_overheated_longs"):
        bias = "funding_overheated"

    elif funding_state in (
        "negative_funding",
        "short_pressure",
        "extreme_short_pressure",
    ):
        bias = "funding_negative"

    return {
        "background_score": round_or_none(scores.get("derivatives_background_score"), 4),
        "per_symbol": per_symbol,
        "aggregate": {
            "avg_funding_zscore": round_or_none(avg_funding_z, 4),
            "funding_state": funding_state,
            "avg_open_interest_change_zscore": round_or_none(avg_oi_z, 4),
            "open_interest_state": oi_state,
            "avg_taker_ratio": round_or_none(avg_taker_ratio, 6),
            "avg_taker_zscore": round_or_none(avg_taker_z, 4),
            "taker_state": taker_state,
            "derivatives_bias": bias,
        },
        "note": (
            "Uwaga: obecny 'derivatives_background_score' w score_all jest liczony głównie na SOL. "
            "Ten raport agreguje dodatkowo BTC/ETH/SOL bez zmiany logiki wyboru profilu."
        ),
    }


PROFILE_TO_MARKET_STATE = {
    "BASE_PROFILE_FALLBACK_NEUTRAL": "unknown_or_mixed",
    "BASE_PROFILE_HIGH_VOL_STRESS": "high_volatility_stress",
    "BASE_PROFILE_MACRO_RISK_OFF_DEFENSIVE": "macro_risk_off",
    "BASE_PROFILE_BEAR_TREND_DEFENSIVE": "crypto_bear",
    "BASE_PROFILE_BEAR_BOUNCE_CAUTIOUS": "crypto_bear_bounce",
    "BASE_PROFILE_RANGE_COMPRESSION_WAIT": "range_compression",
    "BASE_PROFILE_RANGE_CHOP_SELECTIVE": "range_chop",
    "BASE_PROFILE_BULL_TREND_MOMENTUM": "crypto_bull",
    "BASE_PROFILE_BULL_PULLBACK_CONSTRUCTIVE": "crypto_bull_pullback",
    "BASE_PROFILE_ALTSEASON_ROTATION": "altseason_rotation",
    "BASE_PROFILE_BTC_LEADERSHIP_ALTS_WEAK": "btc_leadership_alts_weak",
    "BASE_PROFILE_SOL_RECOVERY_SELECTIVE": "sol_recovery",
}
...
def build_market_state_report(
    ...
    """
    Buduje osobny, czytelny raport stanu rynku, aktywów i pochodnych.
    """
    pinfo = PROFILES.get(profile_id, {})

    assets_by_group = collect_assets_by_group(data, scores)
    derivatives = collect_derivatives_detail(data, scores)
    ...
    market_state_id = PROFILE_TO_MARKET_STATE.get(profile_id, "unknown")

    good_count = sum(1 for q in quality if q.get("status") == "GOOD")
    poor_count = sum(1 for q in quality if q.get("status") == "POOR")
    missing_count = sum(
        1 for q in quality if q.get("status") in ("MISSING_FILE", "MISSING")
    )

    market_summary_pl = (
        f"Reżim: {pinfo.get('display_name', profile_id)}. "
        f"Trend krypto: {interpret_trend_state(crypto_trend)}, "
        f"SOL trend: {interpret_trend_state(sol_trend)}, "
        f"makro: {interpret_macro_state(macro)}, "
        f"zmienność: {interpret_volatility_state(vol)}, "
        f"płynność: {interpret_liquidity_state(liquidity)}."
    )

    spot_state = {
        "BTC": summarize_asset(data, scores, "btc_usd_1d"),
        "ETH": summarize_asset(data, scores, "eth_usd_1d"),
        "SOL": summarize_asset(data, scores, "sol_usd_1d"),
        "ETH/BTC": summarize_asset(data, scores, "eth_btc_1d"),
        "SOL/BTC": summarize_asset(data, scores, "sol_btc_1d"),
    }

    sol_focus = {
        "trend_score": round_or_none(sol_trend, 4),
        "trend_state": interpret_trend_state(sol_trend),
        "relative_strength_vs_btc_score": round_or_none(
            scores.get("sol_relative_strength_score"), 4
        ),
        "relative_strength_state": interpret_trend_state(
            scores.get("sol_relative_strength_score")
        ),
        "recovery_score": round_or_none(sol_recovery, 4),
        "spot_daily": summarize_asset(data, scores, "sol_usd_1d"),
        "spot_hourly": summarize_asset(data, scores, "sol_usd_1h"),
        "pair_daily": summarize_asset(data, scores, "sol_btc_1d"),
        "derivatives": derivatives.get("per_symbol", {}).get("SOL", {}),
    }
    ...
    return {
        ...
        "profile": {
            "id": profile_id,
            "display_name": pinfo.get("display_name", profile_id),
            "bias": pinfo.get("bias", "neutral"),
            "confidence": round_or_none(confidence, 3),
            "description": pinfo.get("description", ""),
        },
        "market_state": {
            "state": market_state_id,
            "summary_pl": market_summary_pl,
            "description": pinfo.get("description", ""),
            "crypto_trend_score": round_or_none(crypto_trend, 4),
            "crypto_trend_state": interpret_trend_state(crypto_trend),
            "sol_trend_score": round_or_none(sol_trend, 4),
            "sol_trend_state": interpret_trend_state(sol_trend),
            "volatility_score": round_or_none(vol, 4),
            "volatility_state": interpret_volatility_state(vol),
            "macro_risk_score": round_or_none(macro, 4),
            "macro_state": interpret_macro_state(macro),
            "liquidity_score": round_or_none(liquidity, 4),
            "liquidity_state": interpret_liquidity_state(liquidity),
            "alt_rotation_score": round_or_none(alt_rotation, 4),
            "btc_leadership_score": round_or_none(btc_leadership, 4),
            "sol_recovery_score": round_or_none(sol_recovery, 4),
            "tags": tags,
        },
        ...
        "macro_state": {
            "score": round_or_none(macro, 4),
            "state": interpret_macro_state(macro),
            "assets": assets_by_group.get("macro_tradfi", []),
        },
        "liquidity_state": {
            "score": round_or_none(liquidity, 4),
            "state": interpret_liquidity_state(liquidity),
            "assets": assets_by_group.get("stablecoins_liquidity", [])
            + assets_by_group.get("crypto_market_breadth", []),
        },
        "dominance_rotation": {
            "alt_rotation_score": round_or_none(alt_rotation, 4),
            "btc_leadership_score": round_or_none(btc_leadership, 4),
            "assets": assets_by_group.get("dominance", [])
            + assets_by_group.get("crypto_relative_strength", []),
        },
        ...
        "data_quality": {
            ...
            "poor_count": poor_count,
            "missing_count": missing_count,
            "poor_datasets": [q["role"] for q in quality if q.get("status") == "POOR"],
            "missing_datasets": [
                q["role"]
                for q in quality
                if q.get("status") in ("MISSING_FILE", "MISSING")
            ],
        },
        "reasons": reasons,
    }
...
def main():
    ...
    parser.add_argument(
        "--data-dir",
        default="./pobrane_dane_v4",
        help="Katalog z plikami *_znormalizowany.json"
    )
    parser.add_argument(
        "--output",
        default="market_profile_recommendation.json",
        help="Plik wyjściowy z rekomendacją profilu"
    )
    parser.add_argument(
        "--state-output",
        default="market_state_report.json",
        help="Plik JSON z raportem stanu rynku i aktywów"
    )
    parser.add_argument(
        "--verbose",
        action="store_true",
    ...
    # 1. Wczytanie danych
    print(f"\n[1/6] Wczytywanie danych z: {args.data_dir}")
    ...
    # 4. Budowanie wyjścia
    print("\n[4/6] Budowanie wyjścia...")
    output = build_output(scores, profile_id, confidence, quality, tags, reasons)

    # 5. Budowanie raportu stanu rynku
    print("\n[5/6] Budowanie raportu stanu rynku...")
    state_report = build_market_state_report(
        data,
        scores,
        output["tags"],
        profile_id,
    ...
    # 6. Zapis
    print("\n[6/6] Zapis plików...")
    print(f"  Zapis rekomendacji do: {args.output}")

    with open(args.output, "w", encoding="utf-8") as f:
        json.dump(output, f, indent=2, ensure_ascii=False)

    if args.state_output and args.state_output != args.output:
        print(f"  Zapis raportu stanu rynku do: {args.state_output}")
        with open(args.state_output, "w", encoding="utf-8") as f:
            json.dump(state_report, f, indent=2, ensure_ascii=False)
    elif args.state_output == args.output:
        print("  [WARN] --state-output jest taki sam jak --output, pomijam zapis raportu stanu.")
    ...
    print(f"  Stan rynku:  {state_report['market_state']['state']}")

    if args.state_output and args.state_output != args.output:
        print(f"  Raport:      {args.state_output}")
